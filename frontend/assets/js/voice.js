/**
 * ClawFlow — VoiceController  (v3)
 *
 * Flow:
 * 1. Record / type command
 * 2. Transcribe  (POST /api/voice/transcribe)
 * 3. Analyze     (POST /api/voice/analyze)
 * 4. Generate    (POST /api/voice/generate-flow)
 * 5. Preview modal
 * 6. Deploy      (POST /api/workflows/deploy)
 */
class VoiceController {
  ...
  _renderCredSlot(assignment, idx) {
    ...
    if (status === 'found') {
      ...
      // NUEVO: Implementación de Dropdown Select para multiples opciones
      const options = matches.map((m, mi) => `
        <option value="${m.id}" data-name="${escapeHtml(m.name)}" ${mi === 0 ? 'selected' : ''}>
          ${escapeHtml(m.name)} — (${m.estado})
        </option>
      `).join('');
      ...
      contentHtml = `
        ...
        <select class="form-control cred-select" data-idx="${idx}" style="margin-bottom: 10px;">
            ${options}
            <option value="manual">-- Nombre manual de n8n --</option>
        </select>
        
        <div id="credManualWrapper_${idx}" style="display:none; margin-top: 10px;">
          <label style="font-size:12px;color:var(--text-muted);">Nombre exacto en n8n:</label>
          <input type="text" class="form-control cred-manual-input" id="credManual_${idx}"
                 placeholder="Ej. Mi Bot Ventas">
        </div>
      `;
    } else {
        ...
        <div class="cred-not-found-info">
          ...
          <div class="cred-opt-panel" id="credOpt_guide_${idx}" style="display:none">
            <div class="cred-guidance-box">
              ${is_auto_creatable ? '<p class="text-muted" style="font-size:12px;margin-bottom:8px">Puedes agregar el token en la <strong>Bóveda de Credenciales</strong> y ClawFlow lo sincronizará.</p>' : ''}
              ...
              <div style="margin-top:10px">
                <label style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:6px">
                  Escribe el nombre que le diste en n8n:
                </label>
          ...
          <div class="cred-opt-panel" id="credOpt_skip_${idx}" style="display:none">
            <p style="font-size:12px;color:var(--text-muted);margin-top:8px">
              El nodo se creará sin credencial.
            </p>
          </div>
        </div>
      `;
    ...
    for (let idx = 0; idx < assignments.length; idx++) {
      ...
      if (a.status === 'found') {
        ...
        if (selectVal === 'manual') {
          ...
          if (!manualVal) { showToast(`Escribe el nombre manual para ${a.credential_label}`, 'warning'); return null; }
          result.push({
          ...
          const match = a.matches.find(m => String(m.id) === selectVal);
          result.push({
            node_type: a.node_type, credential_type: a.credential_type,
            credential_label: a.credential_label, mode: 'use_stored',
            db_credential_id: parseInt(selectVal) || null,
